...
- **Line-item tables** (purchase/sale forms): `<table class="table" id="itemsTable">` with a template `<tr>` that gets cloned via JS for "+ Add Line", each row independently wired for its own event listeners. Don't build a different repeating-row pattern — copy the clone-and-rewire approach in [templates/sales/form.html](../../../templates/sales/form.html). [templates/purchases/form.html](../../../templates/purchases/form.html) has the more complete required-field logic: a trailing blank row is never required, but the moment the owner touches *any* field in it (`rowHasAnyValue()`), the whole row becomes required — silently dropping a half-filled line the owner clearly meant to add would save a purchase that looks complete on screen but is missing stock. `routes/purchases.py::new_purchase` enforces the same rule server-side (`partial_rows` check) since client-side `required` alone isn't trustworthy. `templates/sales/form.html` doesn't have this yet (only its first row is required) — prefer the purchases-form pattern in any new repeated-row form, and note the gap if you're touching the sales form anyway.
